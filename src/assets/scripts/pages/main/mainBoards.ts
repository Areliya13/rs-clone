...
export class MainBoards {
  ...
  constructor() {
    this.content = getElementBySelector(document, HTMLElement, '.main');
  }
  ...
  createRightSidebar = (): HTMLDivElement => {
    ...
    const createBoardDiv = createHtmlElement('div', {className: 'right-linksButtonDiv'});
    const buttonCreateBoard = createHtmlElement('button', {className: 'right-linksButton', innerHTML: `
  ...
  render() {
    this.content.append(this.createMidContent(), this.createRightSidebar());
  }
}